...
`serpens/bin/serpens-sdd`, если в нём нет собственной метки сгенерированного файла — правленый
...
`serpens/bin/serpens-sdd` в хранилище и в каждом подмодуле, до которого дотягивается
...
Докажите обновление. Сам по себе `"$repo/serpens/bin/serpens-sdd" version` здесь не доказывает ничего:
...
руками файл в `serpens/bin/serpens-sdd` и shim, оставшийся указывать на переехавшую или удалённую
установку.
...
этап 3 выше) пишет оба, но проверьте репозиторий, чей `serpens/testing-stack.md` или
...
`serpens/testing-stack.md` — это то, что читают `spns-tdd`, `spns-debugging`, `spns-test-plan` и
...
закоммитить — недостающий добавляйте слиянием из `system-store-template/gitignore.template`,
...
shim из этапа 3/5: по умолчанию это `"$(git rev-parse --show-toplevel)"/serpens/bin/serpens-sdd`, та же
...
действительно изменило. С этой редакции в `serpens/bin/` лежит только сгенерированный shim, а его байты
...
```bash
git -C "<repo>" log --oneline -1 -- serpens/bin/    # коммит обновления, если он тут вообще появился
```
...
`serpens-sdd uninstall [--repo <path>] [--apply] [--include-history]` откатывает установку.
**По умолчанию — сухой прогон**: печатает каждую запись, которую мог сделать комплект,
проверку владения для неё и то, что было бы сделано; на диске ничего не меняется, пока не
передан `--apply`. Работает и для repo-local установки (один репозиторий), и для системного
хранилища (запускайте из корня хранилища; обходит каждый онбордингованный подмодуль).

И сухой прогон, и `--apply` сначала печатают заголовок простым языком: что uninstall трогает,
а что нет, сгруппированную сводку того, что **будет удалено** (счётчики берутся из реального
плана, например `installed commands/skills: 15`), и что **сохраняется намеренно**
(`serpens/testing-stack.md`/`serpens/port-facts.md`, маркеры `.serpens.yaml`, любые
отредактированные командой файлы, не прошедшие проверку владения). Сухой прогон всегда
заканчивается строкой `Dry run — nothing changed. Re-run with --apply to remove.`. `--apply`
заканчивается итоговой запиской: сначала сбой, если он был, затем счётчики
удалено/пропущено/сохранено, оставшиеся ручные шаги (команда снятия регистрации хранилища из
строки 13, повторно указанная здесь), напоминание выполнить `git status` и закоммитить, и
подтверждение, что `openspec list` по-прежнему работает.

Каждое удаление сначала проходит проверку владения — маркер или побайтовое совпадение с
комплектом — никогда не догадку. Файл, не прошедший проверку, отмечается как **«оставлен на
месте, удалите вручную»**, с точным ручным шагом. Конкретно:

- `lefthook.yml` (или `serpens/lefthook.yml`, если у команды уже был свой конфиг) удаляется,
  только если в нём всё ещё наш сгенерированный маркер; `lefthook uninstall` запускается,
  только если не осталось никакого другого конфига lefthook.
- Блок HARD RULE в файле инструкций порта (`CLAUDE.md`, `AGENTS.md`, …) удаляется по своему
  заголовку-маркеру, никогда не остальной файл; сам файл удаляется, только если после этого
  стал пустым И его создала эта установка.
- `openspec/config.yaml`: запись `references:`, каталог context и добавленные этой установкой
  id в `rules:` удаляются, только если их байты всё ещё точно совпадают с тем, что мы бы
  записали; всё, что человек с тех пор отредактировал, остаётся нетронутым.
- Каждая установленная команда и навык проверяются побайтово против того, что записал бы
  этап 6 (исходник комплекта с подставленными токенами `<openspec>`/`<serpens-sdd>`);
  отредактированная копия сохраняется и указывается в отчёте, но не удаляется.
- `serpens/testing-stack.md`, `serpens/branching.md`, `serpens/delivery.md` и `serpens/port-facts.md`
  **сохраняются** — в них настоящие ответы, а не заглушки — если не передан `--include-history`.
- Маркеры изменений `.serpens.yaml` **никогда не удаляются по умолчанию** — это история
  изменений. `--include-history` удаляет только файлы маркеров, никогда `research.md` и другое
  содержимое изменения.
- `git config serpens.baseBranch` / `serpens.agentDir` снимаются, если установлены.
- Зарегистрированное системное хранилище и его коммиты `.gitmodules`/подмодулей **эта команда
  никогда не откатывает**: она печатает точную команду снятия регистрации `openspec store` для
  ручного запуска и никогда не выполняет её сама.

Здесь ничего не коммитится и не пушится. Проверьте `git status`, оставьте нужное, закоммитьте
сами.
...
- [ ] в каждом репозитории есть сгенерированный `serpens/bin/serpens-sdd`, цель `exec` которого —
