...
3. SINGLE REPO. Place yourself BEFORE creating anything.
   ...
   c. Run `<openspec> new change <change-id>` — skip it when the change folder already exists.
      ...
      `spns-plan`, which asks for them the same way. Append verified facts to research.md as pointers
      ...
      pointer per fact. `spns-test-plan` renders the tester's payloads from the spec's scenarios and
