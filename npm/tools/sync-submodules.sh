...
usage() {
  echo "usage: bash sync-submodules.sh (--inventory <json>|--repos-from -) [--store-root <path>]" >&2
}

INVENTORY=""
REPOS_FROM=""
STORE_ROOT="${SERPENS_SYSTEM_STORE_ROOT:-}"
while [ "$#" -gt 0 ]; do
  case "$1" in
    --inventory) INVENTORY=${2:-}; shift 2 ;;
    --repos-from) REPOS_FROM=${2:-}; shift 2 ;;
...
if [ -n "$INVENTORY" ] && [ -n "$REPOS_FROM" ]; then
  echo "✗ --inventory and --repos-from are mutually exclusive" >&2
  exit 2
fi
if [ -z "$INVENTORY" ] && [ -z "$REPOS_FROM" ]; then
  echo "✗ one of --inventory or --repos-from is required" >&2
  usage
  exit 2
fi
if [ -n "$REPOS_FROM" ] && [ "$REPOS_FROM" != "-" ]; then
  echo "✗ --repos-from only supports '-' (stdin)" >&2
  exit 2
fi
...
NODE
  ...
  # --repos-from -: rows arrive on stdin already in this script's own internal
  # `name<TAB>url<TAB>base_branch` row format (see the per-row validation loop right below,
  # which both input modes now share verbatim) — the caller (stage1's resolved rows, fed by
  # stage4/stage9) has already gone through config.mjs's own name/url/branch validation, so
  # this mode does not re-derive it from a JSON schema the way --inventory does.
  inventory_rows=$(cat)
fi
